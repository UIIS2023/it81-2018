...
public class Controller implements PropertyChangeListener {
	...
	public void mouseClicked(MouseEvent e) throws Error {
		stateChecker(e);
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if(evt.getPropertyName() == "Redo Stack" && (int) evt.getNewValue() > 0) {
			...
			private void drawHexagon(MouseEvent e) { 
				...
				if(dlgHex.isConfirm()) {
					if(checkType(dlgHex.getTxtRadius().getText())) {
						...
						JOptionPane.showMessageDialog(frame,"Illegal input type!",
								"Illegal radius error",
								JOptionPane.ERROR_MESSAGE);
					}
				}
			...
			private void selectShape(MouseEvent e) {
				for(int i = 0; i<model.getbasicShapes().size(); i++)
				{
					if(model.getbasicShapes().get(i).contains(e.getX(), e.getY()))
					{
						if(model.getbasicShapes().get(i).isSelected())
						{
							return;
						}
						Shape shape = model.getbasicShapes().get(i); 
						CmdSelect CmdSelect = new CmdSelect(model, shape); // poziva komandu za sel 
						CmdSelect.execute();
			...
			public void selectShapeFromLog(Shape shape) {
				int index = model.getbasicShapes().indexOf(shape);
				Shape selectedShape = model.getbasicShapes().get(index);
			...
			public void modifybasicShape() throws Error {
						...
						if(dlgHexagonUpdate.isConfirmation()) {
							try {
								if(checkType(dlgHexagonUpdate.getTxtCenterX().getText()) && checkType(dlgHexagonUpdate.getTxtCenterY().getText()) && checkType(dlgHexagonUpdate.getTxtR().getText())) {
									...
									hex.setBorderColor(dlgHexagonUpdate.getOutlineColor());hex.setAreaColor(dlgHexagonUpdate.getFillColor());
			...
			public void toFront() {
				if(model.getSelectedShapes().size() == 1) {
					int index = model.getbasicShapes().indexOf(model.getSelectedShapes().get(0));
					Shape shape = model.getbasicShapes().get(index);
			...
			public void serialize() {
				...
				if (!model.getbasicShapes().isEmpty()) {
...
//					chooser.setFileFilter(new FileNameExtensionFilter("Image", "img"));
				...
				if (!model.getUndoStack().isEmpty() || model.getbasicShapes().isEmpty()) chooser.setFileFilter(new FileNameExtensionFilter("Commands log", "log"));
			...
			public void modifyShape() throws Error {
				//???
			}

			private void stateChecker(MouseEvent e) throws Error {
				if(frame.getState() == 1)
				{
					drawPoint(e);
				}
