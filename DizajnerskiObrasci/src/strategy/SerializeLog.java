...
public class SerializeLog implements OptionChooser {
	...
	public SerializeLog(Frame frame, Model model, Controller controller) {
		this.frame = frame;
		this.model = model;
		...
		try {
			...
			case "Added":
				...
				if (commands[1].split(":")[0].equals("Donut")) {
					...
					controller.executeCommand(new CmdCircleAdd(model, (Circle) shape));
			...
			case "Updated":
				...
				if (oldShape instanceof Point) {
					...
					controller
							.executeCommand(new CmdHexagonUpdate((HexagonAdapter) model.getByIndex(index), newHexagon));
			...
			case "Moved to front":
				...
				controller.executeCommand(new CmdToFront(model,model.getbasicShapes().indexOf(shapeMovedToFront), shapeMovedToFront));
			...
			case "Moved to back": 
				...
				controller.executeCommand(new CmdToBack(model, model.getbasicShapes().indexOf(shapeMovedToBack),shapeMovedToBack));
			...
			case "Brought to back":
				...
				controller.executeCommand(new CmdBringToBack(model,model.getbasicShapes().indexOf(shapeBroughtToBack), shapeBroughtToBack));
	...
	private Shape parseShape(String shape, String shapeParameters) throws Error {
	...
	private Circle parseCircle(String string) throws NumberFormatException, Error {
	...
	private HexagonAdapter parseHexagon(String string) throws Error {
	...
	private Donut parseDonut(String string) throws NumberFormatException, Error {
